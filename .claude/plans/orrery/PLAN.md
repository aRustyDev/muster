...
| 6a | engine surfaces for the app: non-persisting digest dry-run (blocks Muster Alpha slice), analytics + 2-hop co-attendance (blocks Muster Beta slice), 10⁵ budget set defined, anchor producer + consult | phases/06a-engine-surfaces.md pre-committed at entry | complete 2026-08-02 — all six hypotheses confirmed; **Orrery Alpha exit gate met at 10⁵** (release, MemoryRepo — qualifications in the phase doc) *(row added 2026-08-02 by the plan review — this work was promised by ROADMAP/specs but owned by no phase)* |
